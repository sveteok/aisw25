...
class BookReader {
  ...
  printReadingStatus(author: string, title: string): void {
    ...
    const found = this.books.find((b) => {
      return (
        b.author.toLowerCase() === bookAuthor &&
        b.title.toLowerCase() === bookTitle
      );
    });
    if (found) {
      console.log(
        `The book ${author}, ${title} ${
          found.readingStatus ? "was" : "was not"
        } read`
      );
    } else {
...
async function main(): Promise<void> {
  ...
  try {
    ...
    if (bookForReading) {
      bookReader.printReadingStatus(
        bookForReading.author,
        bookForReading.title
      );
    ...
    const newBookId = bookReader.addNewBook(
